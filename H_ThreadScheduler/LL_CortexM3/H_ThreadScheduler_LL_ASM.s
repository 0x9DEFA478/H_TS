...
H_TS_ThreadSwitch PROC
	LDR       R0,=H_TS_RunThread         ;加载Thread地址
	LDR       R2,[R0]                    ;读取原来线程的栈指针地址 &H_TS_RunThread[0]

	PUSH      {R2,LR}
	BL        H_TS_RunThread_Refresh     ;更新当前线程信息
	POP       {R2,LR}

	CMP       R0,#0x00000000             ;结果是否为0
	...
	STR       R0,[R2]                    ;保存栈指针 到原来线程的H_TS_RunThread[0]

	LDR       R0,=H_TS_RunThread
	LDR       R0,[R0]                    ;读取新的栈指针地址 &H_TS_RunThread[0]
	LDR       R0,[R0]                    ;读取栈指针 H_TS_RunThread[0]
...
H_TS_StartFirstThread PROC
	...
	LDR       R1,=H_TS_RunThread         ;加载Thread地址
	LDR       R1,[R1]                    ;读取栈指针地址 &H_TS_RunThread[0]
	LDR       R0,[R1]                    ;读取栈指针 H_TS_RunThread[0]
