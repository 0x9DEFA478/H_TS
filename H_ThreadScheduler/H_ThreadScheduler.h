 ...
 * @LastEditTime: 2022-06-24 16:46:32
...
typedef struct{
  ...
  volatile Hsize AddFlag;                        //当被Give成功, 该段清零 (H_TS利用此段在某些情况下可以判断内部是否有信号量)
...
typedef struct{
  ...
  volatile Hsize AddFlag;                        //当被Send成功, 该段清零 (H_TS利用此段在某些情况下可以判断内部是否有消息)
...
typedef struct{
  H_TS_Thread* volatile Owner;                   //因读取队列而阻塞的线程
  volatile Hsize AddFlag;                        //当被Post成功, 该段清零 (H_TS利用此段在某些情况下可以判断内部是否有数据)
