 ...
 * @LastEditTime: 2022-01-12 18:44:13
...
typedef struct{
  ...
  //等待添加到Threads中的线程
  H_TS_Thread* volatile WaitAddThreads;


  //就绪线程链表
  H_TS_Thread* volatile ReadyThreads;
  //副就绪线程链表 能使高优先级的线程免于因为低优先级解阻塞导致过多查询链表的麻烦 用于快速放置低优先级的就绪线程(直接添加到头部 不进行排序, 在SubReadyThreads有高于ReadyThreads优先级的线程时才开始排序添加到ReadyThreads)
  H_TS_Thread* volatile SubReadyThreads;
  //副就绪线程链表中最高优先级的线程的优先级 当SubReadyThreads不为NULL时有效
  volatile Hsize SubReadyThreadsHighestPriority;


  //当此段不为0时SleepThreads有更新
  volatile Hsize SleepThreadsEvent;
  //睡眠线程链表
  H_TS_Thread* volatile SleepThreads;


  //当此段不为0时TimeBlockThreads有更新
  volatile Hsize TimeBlockThreadsEvent;
  //时间限制阻塞线程链表
  H_TS_Thread* volatile TimeBlockThreads;


  //当此段不为0时BlockThreads有更新
  volatile Hsize BlockThreadsEvent;
#if vH_TS_BlockThreadListNum > 1
  //下一个阻塞线程要加到的链表ID
  volatile Hsize BlockThreadsNextAddIndex;
  //当此段不为0时对应的BlockThreads有更新
  volatile Hsize BlockThreadsEvents[vH_TS_BlockThreadListNum];
  //阻塞线程链表(多个链表)
  H_TS_Thread* volatile BlockThreads[vH_TS_BlockThreadListNum];
#else
  //阻塞线程链表
  H_TS_Thread* volatile BlockThread;
#endif


  //解阻塞线程快速通道
  H_TS_UnblockThread UnblockThread;

  //时间限制解阻塞快速通道
  H_TS_UnblockThread TimeUnblockThread;
