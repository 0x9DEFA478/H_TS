...
void IP_USART_Transmit(IP_USART* ip_usart,void* Buffer,int Length){
  ...
  cIP_USART_txDMA(ip_usart)->CCR&=(~DMA_CCR_EN);//失能DMA
  ...
  if(TransmitLastLength>0){
    ...
    cIP_USART_txDMA(ip_usart)->CMAR=(unsigned int)ip_usart->TransmitBuffer;

    if(TransmitLastLength>TransmitBufferSize){
      cIP_USART_txDMA(ip_usart)->CNDTR=TransmitBufferSize;
      ...
      cIP_USART_txDMA(ip_usart)->CNDTR=TransmitLastLength;
    ...
    cIP_USART_txDMA(ip_usart)->CCR|=DMA_CCR_EN;//重新启动DMA
  }
...
void IP_USART_IRQ(IP_USART* ip_usart){
  ...
  if((SR&USART_SR_IDLE)!=0){
    ...
    cIP_USART_rxDMA(ip_usart)->CCR&=(~DMA_CCR_EN);
    oIP_DMA_ClearAllBit_IFCR(ip_usart->Instance.DMA_Rx);

    Length=ip_usart->ReceiveBufferHaftSize-cIP_USART_rxDMA(ip_usart)->CNDTR;

    if(ip_usart->IsOdd==0){
      Buffer=ip_usart->ReceiveBuffer;
      cIP_USART_rxDMA(ip_usart)->CMAR=(unsigned int)&ip_usart->ReceiveBuffer[ip_usart->ReceiveBufferHaftSize];
      ip_usart->IsOdd=-1;
    }else{
