 ...
 * @LastEditTime: 2021-09-30 21:42:03
...
int IP_SPI_isBusy_M_onlyTx_dma(IP_SPI* ip_spi){
  ...
  if(!(ip_spi->Feature&vIP_SPI_Feature_isITDMA)){
    if(cIP_SPI_txDMA(ip_spi)->CCR&DMA_CCR_EN){
...
int IP_SPI_isBusy_M_TxRx_dma(IP_SPI* ip_spi){
  ...
  if(!(ip_spi->Feature&vIP_SPI_Feature_isITDMA)){
    if(cIP_SPI_rxDMA(ip_spi)->CCR&DMA_CCR_EN){
...
int IP_SPI_SetBitWidth(IP_SPI* ip_spi,int BitWidth){
  ...
  switch(BitWidth){
    case 8:
      ...
      if(ip_spi->Feature&vIP_SPI_Feature_DMA)
      {
        if(ip_spi->Feature&vIP_SPI_Feature_Tx){
          cIP_SPI_txDMA(ip_spi)->CCR=(cIP_SPI_txDMA(ip_spi)->CCR&(~(DMA_CCR_MSIZE_Msk|DMA_CCR_PSIZE_Msk)))|((0U<<DMA_CCR_MSIZE_Pos)|(0U<<DMA_CCR_PSIZE_Pos));
        }
        if(ip_spi->Feature&vIP_SPI_Feature_Rx){
          cIP_SPI_rxDMA(ip_spi)->CCR=(cIP_SPI_txDMA(ip_spi)->CCR&(~(DMA_CCR_MSIZE_Msk|DMA_CCR_PSIZE_Msk)))|((0U<<DMA_CCR_MSIZE_Pos)|(0U<<DMA_CCR_PSIZE_Pos));
        }
      }
    ...
    case 16:
      ...
      {
        if(ip_spi->Feature&vIP_SPI_Feature_Tx){
          cIP_SPI_txDMA(ip_spi)->CCR=(cIP_SPI_txDMA(ip_spi)->CCR&(~(DMA_CCR_MSIZE_Msk|DMA_CCR_PSIZE_Msk)))|((1U<<DMA_CCR_MSIZE_Pos)|(1U<<DMA_CCR_PSIZE_Pos));
        }
        if(ip_spi->Feature&vIP_SPI_Feature_Rx){
          cIP_SPI_rxDMA(ip_spi)->CCR=(cIP_SPI_txDMA(ip_spi)->CCR&(~(DMA_CCR_MSIZE_Msk|DMA_CCR_PSIZE_Msk)))|((1U<<DMA_CCR_MSIZE_Pos)|(1U<<DMA_CCR_PSIZE_Pos));
        }
...
void IP_SPI_DMATx_bytes(IP_SPI* ip_spi,unsigned char* TxData,int Len){
  ...
  cIP_SPI_txDMA(ip_spi)->CCR&=(~DMA_CCR_EN);//失能DMA

  oIP_DMA_ClearAllBit_IFCR(ip_spi->Instance.DMA_Tx);

  cIP_SPI_txDMA(ip_spi)->CNDTR=(unsigned int)Len;
  cIP_SPI_txDMA(ip_spi)->CMAR=(unsigned int)TxData;

  cIP_SPI_txDMA(ip_spi)->CCR|=DMA_CCR_EN;//使能DMA
}
...
void IP_SPI_DMATxRx_bytes(IP_SPI* ip_spi,unsigned char* TxData,unsigned char* RxData,int Len){
  ...
  cIP_SPI_txDMA(ip_spi)->CCR&=(~DMA_CCR_EN);//失能DMA
  cIP_SPI_rxDMA(ip_spi)->CCR&=(~DMA_CCR_EN);//失能DMA
  ...
  cIP_SPI_rxDMA(ip_spi)->CNDTR=(unsigned int)Len;
  cIP_SPI_rxDMA(ip_spi)->CMAR=(unsigned int)RxData;

  cIP_SPI_rxDMA(ip_spi)->CCR|=DMA_CCR_EN;//使能DMA
  cIP_SPI_txDMA(ip_spi)->CCR|=DMA_CCR_EN;//使能DMA
}
...
static void IP_SPI_DMATx_IT_bytes_TransStatusRefresh(IP_SPI* ip_spi){
  ...
  if(ip_spi->TransStatus.RemainingSize==0){
    ...
    cIP_SPI_txDMA(ip_spi)->CCR&=(~DMA_CCR_TCIE);//失能完成中断
    ip_spi->TransStatus.Busy=0;//解除忙碌状态
    TransDoneITCallback=ip_spi->TransDoneITCallback;
    ...
    if(ip_spi->TransStatus.RemainingSize>65535){
      cIP_SPI_txDMA(ip_spi)->CNDTR=65535U;
      cIP_SPI_txDMA(ip_spi)->CMAR=(unsigned int)ip_spi->TransStatus.Tx_Ptr;

      ip_spi->TransStatus.RemainingSize-=65535;
      ip_spi->TransStatus.Tx_Ptr=&((unsigned char*)ip_spi->TransStatus.Tx_Ptr)[65535];
    }else{
      cIP_SPI_txDMA(ip_spi)->CNDTR=(unsigned int)ip_spi->TransStatus.RemainingSize;
      cIP_SPI_txDMA(ip_spi)->CMAR=(unsigned int)ip_spi->TransStatus.Tx_Ptr;
...
static void IP_SPI_DMATxRx_IT_bytes_TransStatusRefresh(IP_SPI* ip_spi){
  ...
  if(ip_spi->TransStatus.RemainingSize==0){
    ...
    cIP_SPI_rxDMA(ip_spi)->CCR&=(~DMA_CCR_TCIE);//失能完成中断
    ip_spi->TransStatus.Busy=0;//解除忙碌状态
    TransDoneITCallback=ip_spi->TransDoneITCallback;
    TransDoneITCallback(ip_spi->v_TransDoneITCallback);//调用用户完成回调
    ...
    if(ip_spi->TransStatus.RemainingSize>65535){
      cIP_SPI_txDMA(ip_spi)->CNDTR=65535U;
      cIP_SPI_txDMA(ip_spi)->CMAR=(unsigned int)ip_spi->TransStatus.Tx_Ptr;
      cIP_SPI_rxDMA(ip_spi)->CNDTR=65535U;
      cIP_SPI_rxDMA(ip_spi)->CMAR=(unsigned int)ip_spi->TransStatus.Rx_Ptr;
      ...
      cIP_SPI_txDMA(ip_spi)->CNDTR=(unsigned int)ip_spi->TransStatus.RemainingSize;
      cIP_SPI_txDMA(ip_spi)->CMAR=(unsigned int)ip_spi->TransStatus.Tx_Ptr;
      cIP_SPI_rxDMA(ip_spi)->CNDTR=(unsigned int)ip_spi->TransStatus.RemainingSize;
      cIP_SPI_rxDMA(ip_spi)->CMAR=(unsigned int)ip_spi->TransStatus.Rx_Ptr;
...
void IP_SPI_DMATx_IT_bytes(IP_SPI* ip_spi,unsigned char* TxData,int Len){
  ...
  cIP_SPI_txDMA(ip_spi)->CCR|=DMA_CCR_TCIE;//使能完成中断
...
void IP_SPI_DMATxRx_IT_bytes(IP_SPI* ip_spi,unsigned char* TxData,unsigned char* RxData,int Len){
  ...
  cIP_SPI_rxDMA(ip_spi)->CCR|=DMA_CCR_TCIE;//使能完成中断
